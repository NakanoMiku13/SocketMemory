...
{   
    ...
    async static Task Main(string[] args)
    {
        ...
        Console.CancelKeyPress += (sender, e) => {
            e.Cancel = true;
            logger.LogCritical("Closing program");
            cancellationToken.Cancel();
        };
        AppDomain.CurrentDomain.ProcessExit += (sender, e) =>
        {
            logger.LogCritical("Application exiting. Cancelling tasks...");
            cancellationToken.Cancel();
        };
        try{
            ...
            if(isServer){
                ...
                ServerSocket listener = new(token, ip, port);
                try{
                    logger.LogInformation($"Server started at: {ip}:{port}");
                    while(!token.IsCancellationRequested){
                        logger.LogInformation($"Waiting for new connection... {token.IsCancellationRequested}");
                ...
                ClientSocket socket = new(serverIp, port, cancellationToken.Token);
                ...
                try{
                    ...
                    if(response.Contains(Constants.ACK)){
                        ...
                        if(response.Contains(Constants.START)){
                            difficult = Convert.ToInt32(response.Split("-")[1]);
                        }
                        string id = "";
                        do{
                        ...
                        if(response.Contains(Constants.PLAYER_ID)){
                            ...
                            while(!endGame){
                                if(response.Contains(Constants.ERROR)){
                                    ...
                                    break;
                                }else if(response.Contains(Constants.CLEAR)){
                                ...
                                if(response.Contains(Constants.COORD)){
                                    ...
                                    if(!words.ContainsKey(unzipped[1])){
                                        ...
                                        lastCoord = unzipped[1];
                                    }
                                    PrintMap(difficult, words);
                                }
                                if(!response.Contains(Constants.CLEAR))turn = response.Split("-")[1];
                            }
                        }
                    }else{
